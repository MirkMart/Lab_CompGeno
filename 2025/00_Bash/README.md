...
In the top right corner, there is the menu of NoMachine with a couple of configurable options.

## Getting bash

First of all it is mandatory to have a shell that where we can exercise. Linux and Mac users already have one created by default by their operative systems. Differently, Windows users must download an emulator of a linux terminal. There are many different available options, but I recommend [git bash](https://git-scm.com/downloads/win) (this program has both a unix terminal and git itself. We will come back latere here to download git also for Linux and Mac).
...
There are other commands that will be further discusssed in the next section. Here are listed in order to have a comprehensive view of all possibile used commands in a single resource.

---

### Git and GitHub

More on the topic is present in the [dedicated Git page](./Git_GitHub.md)
